...
def _plot_single_heatmap(location: str, plot_df: pd.DataFrame, suffix: str):
    ...
    fig, ax = plt.subplots(figsize=(1.4 * len(plot_df.columns) + 2, 0.45 * len(plot_df) + 2.5))
...
def plot_heatmap(location: str, metrics=None):
    ...
    groups = {
        "localcv": ["raw"] + [m for m in df["method"] if m.startswith("localcv_")],
        "transfer": ["raw"] + [m for m in df["method"] if m.startswith("transfer_")],
    }
    ...
    for suffix, keep in groups.items():
        ...
        if sub.empty:
            print(f"Skipped {location} {suffix}: no matching methods")
            continue
...
def main():
    ...
    parser.add_argument("--locations", nargs="+", default=["fedjeosen", "fauskane"])
